...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Brighttravel account.<br>
        ...
        &mdash; Brighttravel Team
      `
      return sendMail({ toEmail: email, subject: 'Brighttravel - Password Reset', content })
    })
